...
class TestInterviewEndpoint:
    ...
    def test_successful_chat_returns_ai_response(self, client, mock_db_session, sample_story):
        ...
        with patch("backend.app.api.endpoints.interview.get_db") as mock_get_db:
            ...
            with patch("backend.app.services.interview.agent_app") as mock_agent:
                ...
                response = client.post(
                    f"/api/interview/{sample_story.id}",
                    json={"message": "Hello!"}
                )
    ...
    def test_phase_context_from_unknown_defaults_to_unknown(self, client, mock_db_session, sample_story):
        ...
        with patch("backend.app.api.endpoints.interview.get_db") as mock_get_db:
            ...
            with patch("backend.app.services.interview.agent_app") as mock_agent:
                ...
                with patch("backend.app.services.interview.InterviewService.process_chat") as mock_process:
                    mock_message = Mock()
                    mock_message.id = 1
