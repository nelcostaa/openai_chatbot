...
from backend.app.services.interview import InterviewService, PHASE_PROMPTS
...
class TestInterviewService:
    ...
    def test_process_chat_loads_conversation_history(self, mock_db_session, sample_story):
        """Should load previous messages as context."""
        from backend.app.models.message import Message
        
        # Create existing messages
        msg1 = Message(story_id=sample_story.id, role="user", content="First message", phase_context="GREETING")
        msg2 = Message(story_id=sample_story.id, role="assistant", content="First response", phase_context="GREETING")
    ...
    def test_process_chat_uses_correct_phase_instruction(self, mock_db_session, sample_story):
    ...
    def test_process_chat_uses_default_phase_for_unknown(self, mock_db_session, sample_story):
    ...
    def test_process_chat_limits_history_to_20_messages(self, mock_db_session, sample_story):
    ...
    def test_process_chat_commits_immediately_after_user_message(self, mock_db_session, sample_story):
